...
proc sheet_test(sheetId: string, accessToken: string): Future[
        string] {.async.} =
...
proc getPlan(uid: string, dt: DateTime): Future[(string, int, seq[
  ...
  info "Current plan: " & currentDay & "    row: " & $idx2 & "    old: " &
          old.join()
...
proc setResultValue(uid: string, row: int, col: char, oldText,
        res: string) {.async.} =
  if res.len == 0:
    return
  ...
  let res = await client.bearerRequest(sheetApi & "/" & sheetId & "/values/" &
      valueRange & "?valueInputOption=" & valueInputType
    , accessToken,
    httpMethod = HttpPut, body = $jReq,
    extraHeaders = headers).withTimeoutEx()
...
proc getKm(activities: seq[JsonNode]): string =
  let a = activities.mapIt(it{"distance"}.getFloat()).mapIt(it / 1000).mapIt(
          it.int).reversed()
  if a.len > 0:
...
proc getDetailedCalories(uid: string, id: BiggestInt): Future[int] {.async.} =
  ...
  let res = await client.bearerRequest(stravaApi & "/activities/" & $id,
          stravaAccessToken).withTimeoutEx()
  let body = await res.body()
  let j = parseJson(body)
  return j["calories"].getFloat().int

proc getKilojoules(uid: string, activities: seq[JsonNode]): Future[
        string] {.async.} =
  var kjs = newSeq[int]()
  for a in activities:
...
proc getActivities(uid: string, dt: DateTime, stravaPagesMax: int): Future[seq[
  ...
  for page in 1..stravaPagesMax:
    let res = await client.bearerRequest(stravaApi &
            "/athlete/activities?page=" & $page & "&per_page=" &
            $stravaPageLimit, stravaAccessToken).withTimeoutEx()
    let body = await res.body()
    let j = parseJson(body)
    ...
    let foundDaysActivities = elems.filterIt(it["start_date"].getStr(
            "").startsWith(today_str))

    for fName in walkFiles("*.json"):
      removeFile fName
...
proc getBikeActivities(uid: string, activities: seq[JsonNode]): Future[seq[(
  ...
  for activityI, currentActivity in foundActivities:
    ...
    var file = openAsync("2_" & $(activityI+1) & ".json", fmWrite)
    await file.write(j2.pretty)
    file.close()

    let actName = j2["name"].getStr() & " " & $j2["distance"].getFloat() &
            "m " & j2["type"].getStr()

    if j2["device_watts"].getBool():
      debug "get: " & stravaApi & "/activities/" & $id & "/streams/time,watts?resolution=high&series_type=time"

      let res3 = await client.bearerRequest(stravaApi & "/activities/" &
                      $id & "/streams/time,watts?resolution=high&series_type=time",
                      stravaAccessToken).withTimeoutEx()
      let body3 = await res3.body()
      let j3 = parseJson(body3)

      file = openAsync("3_" & $(activityI+1) & ".json", fmWrite)
      await file.write(j3.pretty)
      file.close()

      let t = j3.getElems().map(x => (x["type"].getStr, x[
              "data"].getElems().map(y => y.getFloat))).toTable
  ...
  if exp-1 < now:
    ...
    let res =
      if prefix == "":
                ...
                clientSecret, refreshToken, clientScope,
                        useBasicAuth = false).withTimeoutEx()
            elif prefix == "strava_":
                await client.refreshToken(stravaAccessTokenUrl, stravaClientId,
                stravaClientSecret, refreshToken, stravaClientScope,
                        useBasicAuth = false).withTimeoutEx()
            else:
...
proc process(testRun: bool, today: DateTime, uid, email: string,
        stravaPagesMax: int) {.async.} =
  let fmt = "yyyy-MM-dd"
